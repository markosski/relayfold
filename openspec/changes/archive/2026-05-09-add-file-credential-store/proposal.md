...
- Add a worker credential store implementation that reads credentials from `~/.relayfold/file_credentials.json`, containing a flat key/value object.
...
- Document the credential file location, JSON structure, and read-only `~/.relayfold` container mount.
...
- Affected code:
  ...
  - `worker/src/index.ts`
  - `worker/README.md`
- The worker reads credentials from `~/.relayfold/file_credentials.json` by default.
- Container deployments should mount `~/.relayfold` read-only for the worker.
