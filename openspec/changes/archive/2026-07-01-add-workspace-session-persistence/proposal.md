...
- Require workers to configure `RELAYFOLD_WORKER_HOST_ID` and advertise that stable host identity during registration.
...
- `worker-pool-ipc`: Worker registration requires stable host identity from `RELAYFOLD_WORKER_HOST_ID`, and task dispatch adds workflow pin constraints and affinity-aware task claiming.
