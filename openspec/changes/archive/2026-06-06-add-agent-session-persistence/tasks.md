...
- [x] 2.1 Derive reusable Agent session keys in the worker from `workflow_instance_id` and logical `task_def_id`.
...
- [x] 5.1 Add core model serialization tests for `reuse_session` defaulting to true and explicit false values.
- [x] 5.2 Add orchestrator tests proving reusable Agent attempts derive the same logical-task session key.
- [x] 5.3 Add focused tests for the human-input session-handling plumbing available in this change; defer end-to-end human-input submission/resume tests to the human-input API change.
- [x] 5.4 Add orchestrator tests proving verifier-feedback attempts dispatch the same logical-task session key when `reuse_session` is true.
- [x] 5.5 Add worker tests proving missing or unreadable reusable sessions are logged and recover by creating a fresh session.
- [x] 5.6 Add worker session store tests for file-backed create, load, persist, and missing-session behavior.
- [x] 5.7 Add Agent executor tests for initial session creation and continuation session loading.
- [x] 5.8 Add Agent executor tests proving missing continuation sessions recover with a fresh session and preserve normal task result handling.
- [x] 5.9 Defer status/result reporting tests for derived session keys because public session-key visibility is intentionally deferred and transcripts remain unexposed.
- [x] 5.10 Add tests proving `reuse_session = false` does not load prior logical-task session history.

## 6. Documentation

- [x] 6.1 Document durable Agent session persistence under `docs/`, including the distinction between workflow state and Agent conversation state.
- [x] 6.2 Document file-backed worker session store configuration.
- [x] 6.3 Document missing-session recovery behavior and its trade-off with verifier-driven retries.
- [x] 6.4 Update existing docs that describe ask or verifier feedback history injection to explain durable session continuation.
- [x] 6.5 Document future blob-backed session store expectations without requiring that adapter in this change.
- [x] 6.6 Document `reuse_session`, including its default `true` behavior and opt-out semantics.
