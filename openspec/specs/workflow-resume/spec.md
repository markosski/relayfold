...
The system SHALL track claimed task dispatches with in-memory task dispatcher lease metadata while the orchestrator process is running, and SHALL recover abandoned running work from durable workflow state after orchestrator restart.
...
- **WHEN** the orchestrator starts with `RELAYFOLD_USE_GLOBAL_NAMESPACE` enabled or disabled
...
- **AND** it does not depend on `RELAYFOLD_USE_GLOBAL_NAMESPACE`
